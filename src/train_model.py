...
KMEANS_PATH = DAGS_DIR / "models" / "kmeans.pkl"                 # Fixed mapping
MODEL_CHOICE_PATH = DAGS_DIR / "models" / "model_choice.json"    # Fixed mapping
...
K_FINAL_OVERRIDE = 4  # Set to None to always use the silhouette-optimal k instead


def train_model(k_override: int | None = K_FINAL_OVERRIDE) -> Path:
    ...
    # 2. Select K via Silhouette Score
    scores = {}
    ...
    # 3. Train Final Models (K-Means & GMM for comparison)
    ...
    # 5. Save Model Choice Audit Metadata
    # (Note: keys/values cast to standard types so json.dump doesn't crash on numpy types)
    model_choice = {
        "k_final": int(k_final),
    ...
    # 6. Apply clusters to DataFrame & Save
